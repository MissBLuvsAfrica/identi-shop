...
export default async function ContactPage() {
  const contact = await getResolvedContact();
  const mailto = `mailto:${contact.email}`;
  const tel = `tel:${contact.phoneE164}`;
  const whatsappUrl = `https://wa.me/${contact.whatsappE164.replace(/^\+/, '')}?text=Hi! I have a question about ${encodeURIComponent(SITE_NAME)}.`;
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <a href={mailto} className="text-gray-600 hover:text-black">
                  {contact.email}
                </a>
              </div>
            </div>
